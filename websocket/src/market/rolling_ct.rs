...
pub type SymbolRollingResponseStream = Pin<Box<dyn Stream<Item=BinanceResult<SocketPayloadActor<SymbolRollingPayload>>> + Send>>;
...
impl SymbolRollingClient {
    ...
    pub async fn subscribe_symbol_rolling(&mut self, symbol: Symbol, window_size: WindowSize) {
        self.websocket_client.subscribe_single(SymbolRollingWindowStream::new(symbol, window_size)).await.unwrap();
    }
    ...
    pub async fn unsubscribe_symbol_rolling(&mut self, symbol: Symbol, window_size: WindowSize) {
        self.websocket_client.unsubscribe_single(SymbolRollingWindowStream::new(symbol, window_size)).await.unwrap();
    }
...
pub(crate) async fn symbol_rolling_payload_process<P>(trade_response_stream: SymbolRollingResponseStream,
                                                      mut processor: P)
where
...
mod tests {
    ...
    use crate::market::types::symbol_rolling::AllSymbolRollingStream;
    use super::*;
    #[tokio::test]
    async fn test_average_price() {
        ...
        symbol_rolling_client.subscribe_symbol_rolling(Symbol::new("ARKUSDT"), WindowSize::FourHours).await;

        sleep(Duration::from_secs(15)).await;

        symbol_rolling_client.subscribe_symbol_rolling(Symbol::new("FILUSDT"),  WindowSize::FourHours).await;
