...
pub type SymbolBookTickerResponseStream = Pin<Box<dyn Stream<Item=BinanceResult<SocketPayloadActor<SymbolBookTickerPayload>>> + Send>>;
...
impl SymbolBookTickerClient {
    ...
    pub async fn subscribe_symbol_book_ticker(&mut self, symbol: Symbol) {
        self.websocket_client.subscribe_single(SymbolBookTickerStream::new(symbol)).await.unwrap();
    }
    ...
    pub async fn unsubscribe_symbol_book_ticker(&mut self, symbol: Symbol) {
        self.websocket_client.unsubscribe_single(SymbolBookTickerStream::new(symbol)).await.unwrap();
    }
...
pub(crate) async fn symbol_book_ticker_payload_process<P>(trade_response_stream: SymbolBookTickerResponseStream,
                                                          mut processor: P)
where
...
mod tests {
    ...
    async fn test_average_price() {
        ...
        symbol_book_ticker_client.subscribe_kline(Symbol::new("ARKUSDT"), Interval::Second1).await;

        sleep(Duration::from_secs(15)).await;

        symbol_book_ticker_client.subscribe_kline(Symbol::new("FILUSDT"), Interval::Second1).await;

        sleep(Duration::from_secs(20)).await;

        symbol_book_ticker_client.unsubscribe_kline(Symbol::new("FILUSDT"), Interval::Second1).await;
