...
pub type MiniTickerResponseStream = Pin<Box<dyn Stream<Item=BinanceResult<SocketPayloadActor<SymbolMiniTickerPayload>>> + Send>>;
...
impl SymbolMiniTickerClient {
    ...
    pub async fn subscribe_mini_ticker(&mut self, symbol: Symbol) {
        self.websocket_client.subscribe_single(SymbolMiniTickerStream::new(symbol)).await.unwrap();
    }
    ...
    pub async fn unsubscribe_mini_ticker(&mut self, symbol: Symbol) {
        self.websocket_client.unsubscribe_single(SymbolMiniTickerStream::new(symbol)).await.unwrap();
    }
...
pub(crate) async fn symbol_mini_ticker_payload_process<P>(trade_response_stream: MiniTickerResponseStream,
                                                          mut processor: P)
where
...
mod tests {
    ...
    async fn test_average_price() {
        ...
        let mut symbol_mini_ticker_client = BinanceMarketWebsocketClient::symbol_mini_ticker().await;

        symbol_mini_ticker_client.subscribe_mini_ticker(Symbol::new("ARKUSDT")).await;

        sleep(Duration::from_secs(15)).await;

        symbol_mini_ticker_client.subscribe_mini_ticker(Symbol::new("FILUSDT")).await;

        sleep(Duration::from_secs(20)).await;

        symbol_mini_ticker_client.unsubscribe_mini_ticker(Symbol::new("FILUSDT")).await;
